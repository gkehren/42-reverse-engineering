...
**Commande `auth AAAAAAAAAAAA`** :

- Alloue 4 octets pour `auth` puis copie `"AAAAAAAAAAAA"` dans cette mémoire. Cela cause un dépassement de tampon, écrasant `auth[8]` avec `'A'` (valeur ASCII 65).
- `auth[8]` est donc non nul, ce qui est nécessaire pour déclencher l'exécution de `/bin/sh` plus tard dans le code.
